...
async function checkFailedTransactionsDelay() {
  try {
    const userId = 34;
    const delayThreshold = 10; // 10 minutes in milliseconds
    const delayThresholdMs = delayThreshold * 60 * 1000;

    console.log(`\n🔍 Checking Failed Transactions with >${delayThreshold} min delay for User ID: ${userId}`);
    console.log(`⏱️  Looking for transactions with >${delayThreshold} minutes between createdAt and updatedAt`);
    console.log('=' .repeat(80));

    // Find all failed payin transactions for user 34
    const failedTransactions = await PayinTransaction.find({
      'user.user_id': userId,
      status: 'failed'
    }).sort({ createdAt: 1 });

    console.log(`📊 Found ${failedTransactions.length} total failed transactions`);
    ...
    console.log(`🚨 Found ${delayedFailedTransactions.length} failed transactions with >${delayThreshold} min delay`);
    console.log(`📈 ${((delayedFailedTransactions.length / failedTransactions.length) * 100).toFixed(2)}% of failed transactions had delays`);

    if (delayedFailedTransactions.length === 0) {
      console.log('✅ No delayed failed transactions found.');
      return;
    }
    ...
    console.log('\n📋 DELAYED FAILED TRANSACTIONS DETAILS:');
    ...
    console.log('-' .repeat(140));
    console.log('\n💰 SUMMARY OF DELAYED FAILED TRANSACTIONS:');
    console.log('=' .repeat(60));
    console.log(`Total Delayed Failed Transactions: ${delayedFailedTransactions.length}`);
    ...
    console.error('❌ Error checking failed transactions delay:', error);
  } finally {
