...
const PayinReport = () => {
  ...
  const fetchTransactions = async (page = 1) => {
    try {
      ...
      const response = await api.get(`/agent/payin-reports?${params.toString()}`);
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await api.get(`/agent/payin-reports/download?${params.toString()}`, {
        responseType: 'blob'
      });
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: 'amount',
      cell: (value: number) => (
        <span className="font-medium text-success-600">{formatCurrency(value)}</span>
      ),
    },
    {
      header: 'Agent Charge',
            ...
            <div className="mb-6">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search by transaction ID, reference ID, or user name..."
