...
interface Row { id: string; type: string; amount: number; balance_after: number; description: string; created_at: string; [key: string]: unknown }
...
export default function WalletTransactionHistory() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true)
    try {
      const res = await api.get('/user/wallet-transaction-history')
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-start justify-between">
        <div className="page-header mb-0">
          <h1 className="page-title">Wallet Transaction History</h1>
          <p className="page-subtitle">Complete wallet activity</p>
        </div>
      ...
      <DataTable columns={columns} rows={rows} loading={loading} searchKeys={['type', 'description']} emptyMessage="No transactions" />
    </div>
  )
}
