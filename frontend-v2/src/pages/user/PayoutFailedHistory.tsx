...
export default function PayoutFailedHistory() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true)
    try {
      const res = await api.get('/user/payout-failed-history')
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-start justify-between">
        <div className="page-header mb-0">
          ...
          <p className="page-subtitle">All failed payout transactions</p>
        </div>
      ...
      <DataTable columns={columns} rows={rows} loading={loading} searchKeys={['order_id', 'status', 'reason']} emptyMessage="No failed payouts" />
    </div>
  )
}
