...
export default function RegisterUser() {
  ...
  const [form, setForm] = useState({ name: '', user_name: '', email: '', mobile: '', password: '', company_name: '', business_type: '' })

  const set = (key: string, val: string) => setForm((p) => ({ ...p, [key]: val }))

  const handleSubmit = async () => {
    if (!form.name || !form.user_name || !form.password) return toast.error('Name, username and password required')
    setSaving(true)
    try {
      await api.post('/agent/register-user', form)
      toast.success('User registered successfully')
      navigate('/agent/add-users')
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message
      toast.error(msg || 'Failed to register user')
    ...
    <div className="max-w-2xl space-y-5 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div className="page-header mb-0">
          ...
          <p className="page-subtitle">Add a new user under your account</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <TextField
            ...
            InputProps={{ endAdornment: <InputAdornment position="end"><IconButton size="small" onClick={() => setShowPwd(!showPwd)}>{showPwd ? <EyeOff size={15} color="#94A3B8" /> : <Eye size={15} color="#94A3B8" />}</IconButton></InputAdornment> }}
          />
